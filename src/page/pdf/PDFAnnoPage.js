...
import { unlistenWindowLeaveEvent, adjustViewerSize } from "../util/window";
import { saveSpan } from "../../core/src/UI/span";
import * as constants from "../../shared/constants";
import result from "../../../pdfs/result";
/**
...
export default class PDFAnnoPage {
   ...
   */
  initializeViewer(
    initialPDFPath = "../pdfs/P12-1046.pdf",
  ...
  createSpan({ text = null, color = null } = {}) {
    ...
    if (!rects && !highlight && selectedAnnotations.length === 0) {
      ...
      return annoUI.ui.alertDialog.show({
        message: "Select text span or an annotation."
      });
    }
  ...
  createRelation({ type, text = null, color = null } = {}) {
    ...
    const relAnnos = window.iframeWindow.annotationContainer
    ...
    const arrows = window.iframeWindow.annotationContainer
      ...
      .filter(a => {
        return (
          anyOf(a.rel1Annotation.uuid, [first.uuid, second.uuid]) &&
  ...
  clearAllAnnotations() {
    if (window.iframeWindow) {
      window.iframeWindow.annotationContainer
        .getAllAnnotations()
        .forEach(a => a.destroy());
    }
  }
  ...
  createRectAnnotation(options) {
    return window.iframeWindow.PDFAnnoCore.default.RectAnnotation.newInstance(
      options
    );
  }
  ...
  createSpanAnnotation(options) {
    ...
    return window.iframeWindow.PDFAnnoCore.default.SpanAnnotation.newInstance(
      options
    );
  }
  ...
  createRelationAnnotation(options) {
    return window.iframeWindow.PDFAnnoCore.default.RelationAnnotation.newInstance(
      options
    );
  }
  ...
  importAnnotation(paperData, isPrimary) {
    window.iframeWindow.annotationContainer
      .importAnnotations(paperData, isPrimary)
      .then(() => {
  ...
  scrollToAnnotation(id) {
    ...
    if (annotation) {
      ...
      let _y =
        (pageHeight + paddingBetweenPages) * (pageNumber - 1) + y * scale;
      _y -= 100;
  ...
  loadPDFFromServer(url) {
    ...
    //       const pdf = Uint8Array.from(atob(this.response.pdf), c =>
    //         c.charCodeAt(0)
    //       )
    ...
    //   xhr.send()
    // })
    const pdf = Uint8Array.from(atob(result.pdf), c => c.charCodeAt(0));
    const analyzeResult = result.analyzeResult;
    return Promise.resolve({ pdf, analyzeResult });
  }
  ...
  loadAnnoFileFromServer(url) {
    return axios
      .get(`${window.API_ROOT}internal/api/annotations?url=${url}`)
      .then(res => {
        if (res.status !== 200 || res.data.status === "failure") {
          ...
          annoUI.ui.alertDialog.show({
            message: "Failed to load an anno file. url=" + url + reason
          });
  ...
  async checkAnnotationUpdate() {
    ...
    (() => {
      ...
      // The name of Content.
      let pdfFileName =
        this.getCurrentContentFile() && this.getCurrentContentFile().name;
      if (!pdfFileName) {
        return;
        // TODO pdftxtとannoダウンロードは、Viewerが閉じている時には無効化すべし.
      }
      // let annoName = pdfFileName.replace(/\.pdf$/i, '.anno')
      let annoName = pdfFileName.replace(
        /\.pdf$/i,
        "." + constants.ANNO_FILE_EXTENSION
      );
