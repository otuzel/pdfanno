...
async function displayViewer() {
  ...
  // Load a PDF file.
  try {
    let { pdf, analyzeResult } = await window.annoPage.loadPDFFromServer(
      pdfURL
    );
  ...
  annoUI.downloadButton.setup({
    ...
    getDownloadFileName: () => {
      if (parseUrlQuery()["paper_id"]) {
        ...
        return (
          parseUrlQuery()["paper_id"] + "." + constants.ANNO_FILE_EXTENSION
        );
      }
      if (
        window.annoPage.getCurrentContentFile() &&
        window.annoPage.getCurrentContentFile().name
      ) {
